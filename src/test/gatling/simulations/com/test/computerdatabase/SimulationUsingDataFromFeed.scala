...
class SimulationUsingDataFromFeed extends Simulation {

  val feeder = csv("search_data.csv").random
  ...
  val scn = scenario("Search Computer - using data from feed").exec(Search.search)
